...
with app.app_context():
    # Check product 573
    product = Product.query.filter_by(name='573').first()
    ...
    if product:
        ...
        if recent_movements:
            ...
            for movement in recent_movements:
                ...
                print(f"  Warehouse ID: {movement.warehouse_id}")
        else:
            print("\nNo recent warehouse movements found.")
            
        # Check if there are any errors in the WarehouseMovement.create_movement method
        print("\nTesting WarehouseMovement.create_movement method:")
        try:
            # Try to create a test movement (we'll roll it back)
            db.session.begin_nested()  # Create a savepoint
            
            test_movement = WarehouseMovement.create_movement(
                product_id=product.id,
                ...
                notes='Testing movement creation'
            )
            
            print(f"Test movement created successfully: {test_movement.id}")
            
            # Rollback to savepoint
            db.session.rollback()
            print("Test movement rolled back (not committed)")
        except Exception as e:
            db.session.rollback()
            print(f"Error creating test movement: {str(e)}")
    else:
        print("Product '573' not found in database.")
