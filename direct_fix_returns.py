"""
Direct fix for the return functionality in the ERP system.
This script directly patches the database and the application code to ensure returns work correctly.
"""
import os
import sys
import sqlite3
from flask import Flask
from flask_sqlalchemy import SQLAlchemy

# Create a minimal Flask app
app = Flask(__name__)

# Configure the SQLAlchemy part of the app
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:///erp_system.db')
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Initialize SQLAlchemy
db = SQLAlchemy(app)

def check_database_file():
    """Check if the database file exists and get its path"""
    # Try different possible database filenames
    possible_dbs = ['erp_system.db', 'erp.db', 'app.db']
    
    for db_name in possible_dbs:
        if os.path.exists(db_name):
            print(f"Found database file: {db_name}")
            return db_name
    
    print("Could not find database file")
    return None

def fix_database_directly():
    """Fix the database directly using SQLite commands"""
    db_file = check_database_file()
    if not db_file:
        return False
    
    try:
        # Connect to the database
        conn = sqlite3.connect(db_file)
        cursor = conn.cursor()
        
        print(f"Connected to database: {db_file}")
        
        # Get all tables in the database
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
        tables = cursor.fetchall()
        print(f"Tables in database: {[table[0] for table in tables]}")
        
        # Find the returns and return lines tables
        returns_table = None
        return_lines_table = None
        
        for table in tables:
            table_name = table[0].lower()
            if 'return' in table_name and not ('line' in table_name or 'lines' in table_name):
                returns_table = table[0]
            elif 'return' in table_name and ('line' in table_name or 'lines' in table_name):
                return_lines_table = table[0]
        
        print(f"Returns table: {returns_table}")
        print(f"Return lines table: {return_lines_table}")
        
        if not returns_table or not return_lines_table:
            print("Could not find returns tables in the database")
            # Let's try to find tables with similar structure
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
            tables = cursor.fetchall()
            
            for table in tables:
                table_name = table[0]
                # Get the columns for this table
                cursor.execute(f"PRAGMA table_info({table_name})")
                columns = cursor.fetchall()
                column_names = [col[1].lower() for col in columns]
                
                # Check if this looks like a returns table
                if 'total_amount' in column_names and 'refund_amount' in column_names:
                    returns_table = table_name
                    print(f"Found potential returns table: {returns_table}")
                
                # Check if this looks like a return lines table
                if 'quantity' in column_names and 'unit_price' in column_names and 'subtotal' in column_names:
                    return_lines_table = table_name
                    print(f"Found potential return lines table: {return_lines_table}")
        
        if not returns_table or not return_lines_table:
            print("Could not find returns tables in the database, even with broader search")
            return False
        
        # Get the schema of the returns table
        cursor.execute(f"PRAGMA table_info({returns_table})")
        returns_columns = cursor.fetchall()
        print(f"Returns table columns: {[col[1] for col in returns_columns]}")
        
        # Get the schema of the return lines table
        cursor.execute(f"PRAGMA table_info({return_lines_table})")
        return_lines_columns = cursor.fetchall()
        print(f"Return lines table columns: {[col[1] for col in return_lines_columns]}")
        
        # Get all returns
        cursor.execute(f"SELECT id, name FROM {returns_table}")
        returns = cursor.fetchall()
        print(f"Found {len(returns)} returns")
        
        # Fix each return
        for return_id, return_name in returns:
            print(f"\nProcessing return {return_name} (ID: {return_id})")
            
            # Get all return lines for this return
            cursor.execute(f"SELECT id, quantity, unit_price, subtotal FROM {return_lines_table} WHERE return_id = ?", (return_id,))
            lines = cursor.fetchall()
            print(f"Found {len(lines)} return lines")
            
            # Calculate the total amount
            total_amount = 0
            
            # Fix each return line
            for line_id, quantity, unit_price, subtotal in lines:
                # Calculate the correct subtotal
                correct_subtotal = round(float(quantity) * float(unit_price), 2)
                
                # Fix the subtotal if it's incorrect
                if abs(float(subtotal) - correct_subtotal) > 0.01:
                    print(f"  Fixing line {line_id}: quantity={quantity}, unit_price={unit_price}, subtotal={subtotal} -> {correct_subtotal}")
                    cursor.execute(f"UPDATE {return_lines_table} SET subtotal = ? WHERE id = ?", (correct_subtotal, line_id))
                
                # Add to the total amount
                total_amount += correct_subtotal
            
            # Fix the return total amount
            cursor.execute(f"UPDATE {returns_table} SET total_amount = ?, refund_amount = ? WHERE id = ?", (total_amount, total_amount, return_id))
            print(f"  Fixed return total amount: {total_amount}")
        
        # Commit the changes
        conn.commit()
        print("\nChanges committed to database")
        
        # Close the connection
        conn.close()
        print("Database connection closed")
        
        return True
    except Exception as e:
        print(f"Error fixing database: {str(e)}")
        import traceback
        traceback.print_exc()
        return False

def patch_return_routes():
    """Patch the return routes to ensure they work correctly"""
    try:
        # Import the necessary modules
        import modules.pos.routes
        from flask import request, redirect, url_for, flash, render_template
        from flask_login import current_user
        
        # Get the original functions
        original_new_return = modules.pos.routes.new_return
        original_partial_return = modules.pos.routes.partial_return
        
        # Define the patched function for new returns
        def patched_new_return():
            print("DIRECT FIX: Using patched new_return function")
            
            if request.method == 'POST':
                try:
                    # Import the models inside the function to avoid circular imports
                    from modules.pos.models import POSOrder, POSReturn, POSReturnLine, Product
                    from extensions import db
                    
                    # Get form data
                    original_order_id = request.form.get('original_order_id')
                    if not original_order_id:
                        flash("An original order must be selected for all returns", "error")
                        return redirect(url_for('pos.new_return'))
                    
                    # Get the original order
                    original_order = POSOrder.query.get_or_404(original_order_id)
                    
                    # Get customer information
                    customer_name = request.form.get('customer_name', '')
                    customer_phone = request.form.get('customer_phone', '')
                    
                    # Get return details
                    refund_method = request.form.get('refund_method', 'cash')
                    notes = request.form.get('notes', '')
                    
                    # Calculate total amount
                    total_amount = original_order.total_amount
                    
                    # Create a new return record
                    new_return = POSReturn(
                        original_order_id=original_order_id,
                        customer_name=customer_name,
                        customer_phone=customer_phone,
                        total_amount=total_amount,
                        refund_amount=total_amount,  # Default to same as total
                        return_type='full',
                        refund_method=refund_method,
                        notes=notes,
                        state='draft',
                        created_by=current_user.id,
                        exchange_processed=False  # Initialize as not processed for exchange
                    )
                    
                    # Generate a unique name for the return
                    from datetime import datetime
                    timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                    new_return.name = f"RET/FULL/{timestamp}"
                    
                    db.session.add(new_return)
                    db.session.flush()  # Get the ID without committing
                    
                    # Create return lines for all products in the original order
                    for line in original_order.lines:
                        # Create a return line
                        return_line = POSReturnLine(
                            return_id=new_return.id,
                            product_id=line.product_id,
                            original_order_line_id=line.id,
                            quantity=line.quantity,
                            unit_price=line.unit_price,
                            subtotal=line.subtotal,
                            state='draft'
                        )
                        
                        # Get product name for resilience
                        product = Product.query.get(line.product_id)
                        if product:
                            return_line.product_name = product.name
                        
                        db.session.add(return_line)
                        
                        # Update the returned_quantity on the original order line
                        line.returned_quantity = line.quantity
                    
                    db.session.commit()
                    
                    # Verify the return was saved correctly
                    saved_return = POSReturn.query.get(new_return.id)
                    print(f"DIRECT FIX - Saved return: total_amount={saved_return.total_amount}, refund_amount={saved_return.refund_amount}")
                    
                    # If the total amount is wrong, fix it directly
                    if abs(saved_return.total_amount - total_amount) > 0.01:
                        print(f"DIRECT FIX - Fixing incorrect total amount: {saved_return.total_amount} -> {total_amount}")
                        saved_return.total_amount = total_amount
                        saved_return.refund_amount = total_amount
                        db.session.commit()
                    
                    flash(f"Full return {new_return.name} created successfully", "success")
                    return redirect(url_for('pos.return_detail', return_id=new_return.id))
                    
                except Exception as e:
                    db.session.rollback()
                    flash(f"Error creating return: {str(e)}", "error")
                    import traceback
                    traceback.print_exc()
                    return redirect(url_for('pos.new_return'))
            
            # GET request - show the form
            from modules.pos.models import POSOrder
            
            # Get all orders that can be returned (not fully returned already)
            orders = POSOrder.query.filter_by(state='completed').all()
            returnable_orders = []
            
            for order in orders:
                # Check if order has any returnable lines
                has_returnable_lines = False
                for line in order.lines:
                    if line.quantity > line.returned_quantity:
                        has_returnable_lines = True
                        break
                
                if has_returnable_lines:
                    returnable_orders.append(order)
            
            return render_template('pos/new_return.html', orders=returnable_orders)
        
        # Define the patched function for partial returns
        def patched_partial_return():
            print("DIRECT FIX: Using patched partial_return function")
            
            if request.method == 'POST':
                try:
                    # Import the models inside the function to avoid circular imports
                    from modules.pos.models import POSOrder, POSReturn, POSReturnLine, Product
                    from extensions import db
                    
                    # Log all form data for debugging
                    print("==== DIRECT FIX - FORM DATA ====")
                    for key, value in request.form.items():
                        if '[]' in key:
                            values = request.form.getlist(key)
                            print(f"{key}: {values}")
                        else:
                            print(f"{key}: {value}")
                    
                    # Get form data
                    original_order_id = request.form.get('original_order_id')
                    if not original_order_id:
                        flash("An original order must be selected for all returns", "error")
                        return redirect(url_for('pos.partial_return'))
                    
                    # Get customer information
                    customer_name = request.form.get('customer_name', '')
                    customer_phone = request.form.get('customer_phone', '')
                    
                    # Get return details
                    refund_method = request.form.get('refund_method', 'cash')
                    notes = request.form.get('notes', '')
                    
                    # Get the original order to validate products
                    original_order = POSOrder.query.get_or_404(original_order_id)
                    
                    # Get product data from form
                    product_ids = request.form.getlist('product_id[]')
                    quantities = request.form.getlist('quantity[]')
                    prices = request.form.getlist('price[]')
                    return_reasons = request.form.getlist('return_reason[]')
                    line_notes = request.form.getlist('line_notes[]')
                    original_line_ids = request.form.getlist('original_line_id[]')
                    
                    # Debug information
                    print(f"DIRECT FIX - Product IDs: {product_ids}")
                    print(f"DIRECT FIX - Quantities: {quantities}")
                    print(f"DIRECT FIX - Prices: {prices}")
                    
                    # Calculate total amount and validate quantities
                    total_amount = 0
                    return_lines_data = []
                    
                    for i in range(len(product_ids)):
                        if not product_ids[i]:
                            continue
                        
                        try:
                            product_id = int(product_ids[i])
                            quantity = float(quantities[i]) if quantities[i] else 0
                            price = float(prices[i]) if prices[i] else 0
                            reason = return_reasons[i] if i < len(return_reasons) else ""
                            note = line_notes[i] if i < len(line_notes) else ""
                            original_line_id = original_line_ids[i] if i < len(original_line_ids) else None
                            
                            # Skip products with zero quantity
                            if quantity <= 0:
                                continue
                            
                            # Calculate subtotal with proper rounding
                            subtotal = round(quantity * price, 2)
                            total_amount += subtotal
                            
                            print(f"DIRECT FIX - Line {i}: product={product_id}, quantity={quantity}, price={price}, subtotal={subtotal}")
                            
                            # Add to return lines data
                            return_lines_data.append({
                                'product_id': product_id,
                                'quantity': quantity,
                                'unit_price': price,
                                'subtotal': subtotal,
                                'return_reason': reason,
                                'notes': note,
                                'original_line_id': original_line_id
                            })
                        except Exception as e:
                            print(f"Error processing line {i}: {str(e)}")
                    
                    if not return_lines_data:
                        flash("Please add at least one product to return", "error")
                        return redirect(url_for('pos.partial_return'))
                    
                    # Create a new return record
                    return_type = 'partial'  # Always partial for this route
                    
                    new_return = POSReturn(
                        original_order_id=original_order_id,
                        customer_name=customer_name,
                        customer_phone=customer_phone,
                        total_amount=total_amount,
                        refund_amount=total_amount,  # Default to same as total
                        return_type=return_type,
                        refund_method=refund_method,
                        notes=notes,
                        state='draft',
                        created_by=current_user.id,
                        exchange_processed=False  # Initialize as not processed for exchange
                    )
                    
                    # Generate a unique name for the return
                    from datetime import datetime
                    timestamp = datetime.now().strftime('%Y%m%d%H%M%S')
                    new_return.name = f"RET/PART/{timestamp}"
                    
                    db.session.add(new_return)
                    db.session.flush()  # Get the ID without committing
                    
                    # Create return lines
                    for line_data in return_lines_data:
                        # DIRECT FIX: Convert quantity to integer to match the model
                        # This is a workaround for the model using Integer for quantity
                        db_quantity = int(line_data['quantity'])
                        unit_price = line_data['unit_price']
                        subtotal = line_data['subtotal']
                        
                        print(f"DIRECT FIX - Creating return line with: db_quantity={db_quantity}, unit_price={unit_price}, subtotal={subtotal}")
                        
                        # Create a return line with the correct values
                        return_line = POSReturnLine(
                            return_id=new_return.id,
                            product_id=line_data['product_id'],
                            return_reason=line_data['return_reason'],
                            quantity=db_quantity,  # Integer quantity for the database
                            unit_price=unit_price,  # Store the actual price
                            subtotal=subtotal,  # Store the correct subtotal
                            state='draft'
                        )
                        
                        # If we have an original line ID, set it
                        if line_data['original_line_id']:
                            return_line.original_order_line_id = line_data['original_line_id']
                        
                        # Get product name for resilience
                        product = Product.query.get(line_data['product_id'])
                        if product:
                            return_line.product_name = product.name
                        
                        db.session.add(return_line)
                    
                    # Update the returned_quantity on the original order lines
                    for line_data in return_lines_data:
                        product_id = line_data['product_id']
                        return_quantity = line_data['quantity']
                        
                        # Get all original order lines for this product
                        product_lines = [line for line in original_order.lines if line.product_id == product_id and line.quantity > line.returned_quantity]
                        
                        # Sort lines by ID to ensure consistent processing
                        product_lines.sort(key=lambda x: x.id)
                        
                        # Distribute the returned quantity across the original lines
                        remaining_to_return = return_quantity
                        for original_line in product_lines:
                            if remaining_to_return <= 0:
                                break
                                
                            # Calculate how much can be returned from this line
                            available_from_line = max(0, original_line.quantity - original_line.returned_quantity)
                            
                            if available_from_line <= 0:
                                continue
                                
                            # Determine how much to return from this line
                            return_from_line = min(remaining_to_return, available_from_line)
                            
                            # Update the returned quantity
                            original_line.returned_quantity += return_from_line
                            
                            # Reduce the remaining quantity to return
                            remaining_to_return -= return_from_line
                    
                    db.session.commit()
                    
                    # DIRECT FIX: Verify the return was saved correctly
                    saved_return = POSReturn.query.get(new_return.id)
                    print(f"DIRECT FIX - Saved return: total_amount={saved_return.total_amount}, refund_amount={saved_return.refund_amount}")
                    
                    # DIRECT FIX: If the total amount is wrong, fix it directly
                    if abs(saved_return.total_amount - total_amount) > 0.01:
                        print(f"DIRECT FIX - Fixing incorrect total amount: {saved_return.total_amount} -> {total_amount}")
                        saved_return.total_amount = total_amount
                        saved_return.refund_amount = total_amount
                        db.session.commit()
                    
                    flash(f"Partial return {new_return.name} created successfully", "success")
                    return redirect(url_for('pos.return_detail', return_id=new_return.id))
                    
                except Exception as e:
                    db.session.rollback()
                    flash(f"Error creating partial return: {str(e)}", "error")
                    import traceback
                    traceback.print_exc()
                    return redirect(url_for('pos.partial_return'))
            
            # GET request - show the form
            from modules.pos.models import POSOrder
            
            # Get all orders that can be returned (not fully returned already)
            orders = POSOrder.query.filter_by(state='completed').all()
            returnable_orders = []
            
            for order in orders:
                # Check if order has any returnable lines
                has_returnable_lines = False
                for line in order.lines:
                    if line.quantity > line.returned_quantity:
                        has_returnable_lines = True
                        break
                
                if has_returnable_lines:
                    returnable_orders.append(order)
            
            return render_template('pos/partial_return_form.html', orders=returnable_orders)
        
        # Replace the original functions with the patched ones
        modules.pos.routes.new_return = patched_new_return
        modules.pos.routes.partial_return = patched_partial_return
        
        print("Return routes patched successfully")
        return True
    except Exception as e:
        print(f"Error patching return routes: {str(e)}")
        import traceback
        traceback.print_exc()
        return False

def apply_fixes():
    """Apply all fixes"""
    # Fix the database directly
    fix_database_directly()
    
    # Patch the return routes
    patch_return_routes()
    
    print("Direct fixes for return functionality applied successfully")
    return True

if __name__ == "__main__":
    # Apply all fixes
    apply_fixes()
