"""
EMERGENCY DATABASE FIX for the return functionality
This script directly modifies the database to fix the return amounts
"""
import os
import sqlite3
import sys
...
def fix_database():
    """
    Directly fix the database tables related to returns
    """
    ...
    for table in tables:
        table_name = table[0]
        if 'return' in table_name.lower() and not 'line' in table_name.lower():
            returns_table = table_name
        elif 'return' in table_name.lower() and 'line' in table_name.lower():
    ...
    # Check if the returns table has total_amount and refund_amount columns
    total_amount_col = None
    refund_amount_col = None
    
    for col in returns_columns:
        if col[1].lower() == 'total_amount':
            total_amount_col = col[1]
        elif col[1].lower() == 'refund_amount':
            refund_amount_col = col[1]
    
    if not total_amount_col or not refund_amount_col:
        print("Could not find total_amount or refund_amount columns in returns table")
        return False
    
    # Check if the return lines table has quantity, unit_price, and subtotal columns
    quantity_col = None
    unit_price_col = None
    subtotal_col = None
    
    for col in return_lines_columns:
        if col[1].lower() == 'quantity':
            quantity_col = col[1]
        elif col[1].lower() == 'unit_price':
            unit_price_col = col[1]
        elif col[1].lower() == 'subtotal':
            subtotal_col = col[1]
    
    if not quantity_col or not unit_price_col or not subtotal_col:
        print("Could not find quantity, unit_price, or subtotal columns in return lines table")
        return False
    ...
    for return_id, return_name in returns:
        ...
        cursor.execute(f"SELECT id, {quantity_col}, {unit_price_col}, {subtotal_col} FROM {return_lines_table} WHERE return_id = ?", (return_id,))
        ...
        for line_id, quantity, unit_price, subtotal in lines:
            ...
            if abs(float(subtotal) - correct_subtotal) > 0.01:
                ...
                cursor.execute(f"UPDATE {return_lines_table} SET {subtotal_col} = ? WHERE id = ?", (correct_subtotal, line_id))
            
            # Add to the total amount
            total_amount += correct_subtotal
        
        # Fix the return total amount
        cursor.execute(f"UPDATE {returns_table} SET {total_amount_col} = ?, {refund_amount_col} = ? WHERE id = ?", (total_amount, total_amount, return_id))
...
def create_direct_patch():
    """
    Create a direct patch file to fix the return functionality
    """
    patch_file = 'direct_return_patch.py'
...
""")
    ...
    print(f"Created direct patch file: {patch_file}")
    return True
...
if __name__ == "__main__":
    ...
    # Create the direct patch
    create_direct_patch()
    
    print("Emergency database fix completed successfully")
