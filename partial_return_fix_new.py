...
This script patches the existing partial return route to fix the issues.
"""
from flask import Blueprint, redirect, url_for, flash, request
from flask_login import current_user, login_required
from modules.pos.models import POSOrder, POSReturn, POSReturnLine, POSOrderLine, Product
from extensions import db

def apply_partial_return_fix(pos_blueprint):
    """
    Apply the fix to the POS blueprint by replacing the partial_return route.
    """
    # First, remove the existing route if it exists
    for rule in list(pos_blueprint.deferred_functions):
        if hasattr(rule, '__name__') and rule.__name__ == 'partial_return':
            pos_blueprint.deferred_functions.remove(rule)
            print("Removed existing partial_return route")
    
    # Then register our fixed version
    @pos_blueprint.route('/returns/partial', methods=['GET', 'POST'])
    @login_required
    def partial_return():
