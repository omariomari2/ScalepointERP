"""
Clean fix for the partial return functionality in the ERP system.
This script provides a comprehensive fix for the partial return feature.
"""
...
def apply_clean_fix(app):
    """
    Apply a clean fix for the partial return functionality.
    This replaces all previous fixes with a single, clean implementation.
    """
    print("Applying clean fix for partial return functionality...")
    
    # Create a new route for partial returns
    @app.route('/pos/returns/partial', methods=['GET', 'POST'])
    @login_required
    def partial_return():
        """Clean implementation of the partial return functionality"""
        if request.method == 'POST':
            try:
                ...
                if not product_ids or not any(pid for pid in product_ids if pid):
                    ...
                    return redirect(url_for('partial_return'))
    ...
    # Replace the existing route with our clean implementation
    app.view_functions['partial_return'] = partial_return
    
    # Add the route to the POS blueprint's URL map
    from modules.pos.routes import pos
    pos.add_url_rule('/returns/partial', 'partial_return', partial_return, methods=['GET', 'POST'])
    
    print("Clean fix for partial return functionality applied successfully")
    return True

if __name__ == "__main__":
    # This script can be run directly to test the fix
    print("This script should be imported by app.py, not run directly.")
    print("To apply the fix, add the following to app.py:")
    print("from clean_return_fix import apply_clean_fix")
    print("apply_clean_fix(app)")
