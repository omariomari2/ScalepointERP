...
with app.app_context():
    # Check if product 608 exists
    product = Product.query.filter_by(id=608).first()
    
    if product:
        print(f"Product 608: {product.name}")
        print(f"Current stock: {product.stock_quantity}")
        
        # Find all order lines for product 608
        order_lines = POSOrderLine.query.filter_by(product_id=608).all()
        
        print(f"\nFound {len(order_lines)} order lines for product 608")
        ...
        # If product 608 doesn't exist, let's find what product the user is trying to return
        print("Product 608 not found")
        
        # Check recent returns for errors
        recent_returns = POSReturn.query.order_by(POSReturn.return_date.desc()).limit(5).all()
